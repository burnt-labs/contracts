...
impl Config {
    fn from_env() -> Self {
        let keys_url = env::var("KEYS_URL")
            .unwrap_or_else(|_| "https://verifier.opacity.network/api/public-keys".to_string());
        ...
        let state_path = env::var("STATE_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from(".opacity_keys_state.json"));
...
async fn fetch_keys(url: &str, keys_field: Option<&str>) -> Result<Vec<String>, UpdaterError> {
    ...
    Err(UpdaterError::UnexpectedShape(format!(
        "top-level type: {v}"
    )))
}
...
fn normalize_addr_hex(s: &str) -> Result<String, UpdaterError> {
    ...
    if s.len() != 40 {
        return Err(UpdaterError::BadHexLen);
    }
...
async fn submit_update(cfg: &Config, keys: Vec<String>) -> Result<(), UpdaterError> {
    ...
    let contract_addr = cfg
        .contract_addr
        .clone()
        .ok_or(UpdaterError::MissingContract)?;
    let chain_id = cfg
        .chain_id
        .clone()
    ...
    let chain = ChainInfoOwned {
        chain_id: chain_id.clone(),
        gas_denom: gas_denom.clone(),
        gas_price: 0.0, // overridden by DaemonBuilder::gas_price below
        grpc_urls: vec![grpc.clone()],
        lcd_url: None,
        fcd_url: None,
        network_info: NetworkInfoOwned {
            chain_name: chain_id.clone(),
            pub_address_prefix: prefix.clone(),
            coin_type: 118,
        },
        kind: ChainKind::Unspecified,
    };
    ...
    #[cw_orch::interface(
        ...
        opacity_verifier::msg::ExecuteMsg,
        opacity_verifier::msg::QueryMsg,
        cosmwasm_std::Empty
    )]
    ...
    for k in onchain_keys {
        ...
        let n = normalize_addr_hex(&k).unwrap_or(k);
    ...
    if onchain_set == new_set {
        println!(
            "On-chain allowlist already up to date ({} keys). Skipping submit.",
            new_set.len()
        );
        return Ok(());
    }
...
async fn main() -> Result<(), UpdaterError> {
    ...
    println!(
        "opacity_key_updater started. Polling {} every {}s. Dry-run: {}. Keys field: {}",
        cfg.keys_url,
        cfg.poll_secs,
        cfg.dry_run,
        cfg.keys_field.as_deref().unwrap_or("<top-level array>")
    );
...
async fn run_once(cfg: &Config) -> Result<(), UpdaterError> {
    ...
    if let Some(prev) = prev {
        ...
        println!(
            "No previous state; treating as first run with {} keys.",
            set.len()
        );
        if cfg.dry_run {
            println!(
