...
pub fn time_lock_plugin(ctx: &mut PluginCtx<DefaultXionAssetContext, Empty>) -> StdResult<bool> {
    let (Some(time_lock), Some(reservation)) = (&ctx.data.time_lock, &ctx.data.reservation) else {
        return Ok(true);
    };
    let limit = Expiration::AtTime(ctx.env.block.time.plus_seconds(time_lock.as_secs()));
    if Expiration::AtTime(reservation.reserved_until).gt(&limit) {
        return Err(cosmwasm_std::StdError::generic_err(format!(
            "Reservation end time {} exceeds the collection time lock {}",
            reservation.reserved_until, limit
        )));
    }
    Ok(true)
}
...
pub fn allowed_currencies_plugin(
    ...
    if ctx
        ...
        .ask_price
        .as_ref()
        .is_some_and(|price| !allowed_set.contains(price.denom.as_str()))
    {
        return Err(cosmwasm_std::StdError::generic_err(
    ...
    if ctx
        ...
        .min_price
        .as_ref()
        .is_some_and(|min_price| !allowed_set.contains(min_price.denom.as_str()))
    {
        return Err(cosmwasm_std::StdError::generic_err(
