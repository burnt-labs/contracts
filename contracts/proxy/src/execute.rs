...
use cosmwasm_std::{to_json_binary, Addr, CosmosMsg, Deps, DepsMut, Env, Event, MessageInfo, Response, WasmMsg};
...
pub fn update_admin(deps: DepsMut, info: MessageInfo, new_admin: Option<Addr>) -> ContractResult<Response> {
...
pub fn proxy_msgs(
    ...
    for msg in msgs {
        let (proxy_msg, contract_addr) = match msg {
            WasmMsg::Execute { contract_addr, msg, funds } => (ProxyMsg{sender: info.sender.clone(), msg, funds}, contract_addr),
            _ => return Err(InvalidMsgType)
        };
        let contract_info = deps.querier.query_wasm_contract_info(contract_addr.clone())?;
        if !CODE_IDS.has(deps.storage, contract_info.code_id) {
            return Err(InvalidCodeID {contract: contract_addr, code_id: contract_info.code_id});
        }

        let exec_msg = WasmMsg::Execute {contract_addr, msg: to_json_binary(&proxy_msg)?, funds: proxy_msg.funds};
        proxy_msgs.push(exec_msg);
    }

    Ok(
        Response::new().add_event(Event::new("proxied_msgs")).add_messages(proxy_msgs)
    )
}
...
pub fn add_code_ids(
    ...
    Ok(Response::new().add_event(
        Event::new("updated_proxy_code_ids")
            .add_attributes(vec![
