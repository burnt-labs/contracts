...
fn normalize_addr_hex(s: &str) -> Result<String, crate::error::ContractError> {
    ...
    if s.len() != 40 {
        return Err(crate::error::ContractError::Std(
            cosmwasm_std::StdError::generic_err("address hex must be 40 chars"),
        ));
    }
...
pub fn instantiate(
    ...
    Ok(Response::new().add_event(
        Event::new("create_opacity_verifier").add_attributes(vec![("admin", admin.to_string())]),
    ))
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Verify { signature, message } => to_json_binary(&query::verify_query(
            deps.storage,
            deps.api,
            signature,
            message,
...
mod tests {
    ...
    fn test_verify_proof() {
        ...
        let allowlist_keys_raw = [
            "322df8c3146a9891c8d63deec562db5f325f7a28",
            "3ac1e280b6b5d8e15cf428229eccb20d9b824a53",
            "5a29af4859ebc29ac0819c178bd293ba7f7bdfcf",
            "9b776cbbd434d7d8f32b8cb369c37442760457b5",
            "90cbfa246fb5bd65192aeaaa41483e311a13f109",
            "ae16d88cd1f4ba016da8909ebc7c9c4a4fb112b8",
            "8a4ca92581fb9b569ef8152c70a031569ee971b5",
            "bdd5b7410abf138da1008906191188f4b5543be7",
            "5d92cf96045bb80d869ee7bfa5d894be4782cfab",
            "7775b5ffbcd55e7fce07672895145c5961ff828f",
            "cf203ffb676fad5c8924ceebe91ebe3e617f01af",
        ];
        let allowlist_keys: Vec<String> =
            allowlist_keys_raw.iter().map(|x| x.to_string()).collect();

        let sender = Addr::unchecked("sender");
        // Create a new mock chain (backed by cw-multi-test)
        let chain = Mock::new(&sender);

        let opacity_verifier: OpacityVerifier<Mock> =
            OpacityVerifier::new("opacity_verifier", chain);
        ...
        let verifier_init_msg = InstantiateMsg {
            ...
            admin: "cosmwasm1pgm8hyk0pvphmlvfjc8wsvk4daluz5tgrw6pu5mfpemk74uxnx9qlm3aqg"
                .to_string(),
            allow_list: allowlist_keys,
        };

        opacity_verifier
            .instantiate(&verifier_init_msg, None, &[])
            .unwrap();
